CREATE OR REPLACE TRIGGER BIU_MIEMBRO_MINISTERIO
BEFORE INSERT OR UPDATE ON MIEMBRO_MINISTERIO
FOR EACH ROW
DECLARE l_user VARCHAR2(100);
BEGIN
  l_user := NVL(SYS_CONTEXT('APEX$SESSION','APP_USER'), USER);
  IF INSERTING THEN
    IF :NEW.ID IS NULL THEN SELECT SEQ_MIEMBRO_MINISTERIO.NEXTVAL INTO :NEW.ID FROM dual; END IF;
