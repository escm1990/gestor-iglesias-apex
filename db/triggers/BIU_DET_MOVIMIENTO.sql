CREATE OR REPLACE TRIGGER BIU_DET_MOVIMIENTO
BEFORE INSERT OR UPDATE ON DET_MOVIMIENTO
FOR EACH ROW
DECLARE l_user VARCHAR2(100);
BEGIN
  l_user := NVL(SYS_CONTEXT('APEX$SESSION','APP_USER'), USER);
  IF INSERTING THEN
    IF :NEW.ID IS NULL THEN SELECT SEQ_DET_MOVIMIENTO.NEXTVAL INTO :NEW.ID FROM dual; END IF;
