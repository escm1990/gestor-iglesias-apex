CREATE OR REPLACE TRIGGER BIU_CIERRE_MENSUAL_DET
BEFORE INSERT OR UPDATE ON CIERRE_MENSUAL_DET
FOR EACH ROW
DECLARE l_user VARCHAR2(100);
BEGIN
  l_user := NVL(SYS_CONTEXT('APEX$SESSION','APP_USER'), USER);
  IF INSERTING THEN
    IF :NEW.ID IS NULL THEN SELECT SEQ_CIERRE_MENSUAL_DET.NEXTVAL INTO :NEW.ID FROM dual; END IF;
